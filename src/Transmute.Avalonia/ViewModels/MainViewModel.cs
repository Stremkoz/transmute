...
public record MetadataModeOption(string Label, MetadataMode Value);
...
public partial class MainViewModel : ObservableObject
{
    ...
    [
        new("Preserve all", MetadataMode.PreserveAll),
        new("Strip all", MetadataMode.StripAll),
        new("Colour profile only", MetadataMode.ColorProfile),
        new("Copyright only", MetadataMode.Copyright),
    ];
    ...
    {
        if (value is null) return; // ComboBox can push null transiently while the list is refreshed
    ...
    private void OpenOutputDirectory()
    {
        if (!string.IsNullOrEmpty(OutputDirectory))
            Platform.OpenFolder(OutputDirectory);
    }
    ...
    private async Task ConvertAsync()
    {
        ...
        IReadOnlyList<ConversionResult>? completedResults = null;
        ...
        using (temp)
        {
            ...
            if (sameFormatPaths.Count > 0)
            {
                ...
                var overwriteSame = await MessageDialog.ConfirmAsync(msg, "Transmute — Same Format", MessageIcon.Warning);
                if (!overwriteSame)
                {
        ...
        if (completedResults is not null && config.UI.PlaySoundOnCompletion)
        {
            ...
            Platform.PlayCompletionSound(hasProblems);
        }
    }
